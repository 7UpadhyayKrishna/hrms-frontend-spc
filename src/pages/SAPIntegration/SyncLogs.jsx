...
const SyncLogs = () => {
  ...
  const fetchLogs = async () => {
    try {
      ...
      const response = await api.get('/sap/sync-logs', { params });
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-white">SAP Sync Logs</h1>
      ...
      <div className="bg-gray-800 rounded-lg p-4 mb-6">
        <div className="grid grid-cols-3 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-1">Connection</label>
            <input
              type="text"
              name="connectionId"
              value={filters.connectionId}
              onChange={(e) => setFilters(prev => ({ ...prev, connectionId: e.target.value }))}
          ...
          <div>
            ...
            >
              ...
              <option value="leave">Leave Balance Sync</option>
              <option value="attendance">Attendance Sync</option>
            </select>
          </div>
          <div>
      ...
      <div className="bg-gray-800 rounded-lg overflow-hidden">
        <table className="w-full">
          <thead className="bg-gray-700">
            <tr>
              <th className="px-4 py-3 text-left text-sm font-medium text-gray-300">Connection</th>
            ...
            {logs.map((log) => (
              <tr key={log._id} className="hover:bg-gray-750">
                <td className="px-4 py-3 text-sm text-gray-300">
                  {log.connectionName || log.connection?.connectionName || '-'}
                </td>
